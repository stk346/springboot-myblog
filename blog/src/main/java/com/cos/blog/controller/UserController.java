...
public class UserController {
    ...
    @Value("${cos.key}") // application.yml 제일 밑에 있는 cos: key:를 가져다 쓴다는 의미
    ...
    public String kakaoCallback(String code) { // Data를 리턴해주는 컨트롤러 함수
        ...
        ResponseEntity<String> response2 = rt2.exchange(
                ...
                HttpMethod.POST, // 요청 방식
                kakaoProfileRequest2, // 요청 내용(body)
                String.class //응답 받을 타입
        );
...
//        UUID garbagePassword = UUID.randomUUID(); // UUID란 중복되지 않는 특정 값을 만들어내는 알고리즘 -> 로그인을 할 수 없음
        System.out.println("블로그 서버 패스워드: "+ cosKey); // 카카오로 로그인하는 사용자는 어차피 패스워드가 필요 없으니 랜덤한 패스워드를 넣고 회원가입을 시킨다.
